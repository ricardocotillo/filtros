    ...
    <b-row>
      <b-col md="12">
        <b-table responsive :fields="fields" :items="equivalencia" v-if="equivalencia.length > 0">
          <template v-slot:cell(marca)="data">{{data.value.toUpperCase()}}</template>
        ...
        if (this.codigo.toUpperCase().match(this.inciales[i] + "?")) {
          db.collection("codigos")
            ...
            .where(
              ...
              "<=",
              (
                this.codigo.substring(0, this.codigo.length - 1) +
                String.fromCharCode(
                  this.codigo.charCodeAt(this.codigo.length - 1) + 1
                )
              ).toUpperCase()
            )
            .get()
