    ...
    <b-row align-h="center">
      <b-col md="12">
        <b-table responsive striped hover :items="filtros" :fields="fields" v-if="filtros.length > 0">
          <template #cell(aire)="data">
            <template class="hover" v-for="(item, index) in items(data.value)">
              <a class="hover" @click="mostrarFiltro(item)" v-if="index < items(data.value).length - 1">{{ item + "/" }}</a>
              <a class="hover" @click="mostrarFiltro(item)" v-else>{{ item }}</a>
            </template>
          </template>
          <template #cell(aceite)="data">
            <template v-for="(item, index) in items(data.value)">
              <a class="hover" @click="mostrarFiltro(item)" v-if="index < items(data.value).length - 1">{{ item + "/" }}</a>
              <a class="hover" @click="mostrarFiltro(item)" v-else>{{ item }}</a>
            </template>
          </template>
          <template #cell(combustible)="data">
            <template v-for="(item, index) in items(data.value)">
              <a class="hover" @click="mostrarFiltro(item)" v-if="index < items(data.value).length - 1">{{ item + "/" }}</a>
              <a class="hover" @click="mostrarFiltro(item)" v-else>{{ item }}</a>
            </template>
